...
if ($there_table == 0)
{
	mysql_query("INSERT INTO rating (`id`, `post_id`, `user_id`, `like_post`, `dislike_post`) 
							 VALUES (NULL, '$pid', '$uid', '0', '1')") or die(mysql_error());
}
else
{
	mysql_query("UPDATE rating SET like_post='0', dislike_post='1' WHERE user_id='$uid' AND post_id='$pid'");
}

$post_rating = mysql_fetch_array(mysql_query("SELECT like_post FROM news WHERE id='$pid' LIMIT 1"));

mysql_query("UPDATE news SET dislike_post = dislike_post + 1 WHERE id='$pid'");
if ($post_rating['like_post'] > 0)
{
